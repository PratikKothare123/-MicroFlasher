...
- **Live Deploy URL:** [https://micro-flasher.vercel.app/](https://micro-flasher.vercel.app/)
- **GitHub Repository:** [https://github.com/PratikKothare123/-MicroFlasher](https://github.com/PratikKothare123/-MicroFlasher)

---

## 🔒 Source Code Protection Architecture

1. **Admin Sketch Upload:** Admins upload raw `.ino` files alongside title, description, and target board FQBN (`esp32:esp32`, `arduino:avr`, `esp8266:esp8266`).
2. **Background Compilation:** The Express backend executes `arduino-cli` in an isolated child process to compile the sketch into a `.bin` or `.hex` binary file.
...
│   ├── compilationService.js      # arduino-cli process execution and sketch isolation
│   ├── db.js                      # Database initialization and async query wrappers
│   ├── bin/                       # Compiled binary output
│   ├── data/                      # Local database storage
│   └── uploads/                   # Binary storage directory; .ino source is deleted
└── client/
    ...
    │   ├── esp-flasher.js         # esptool-js wrapper for ESP32 and ESP8266 flashing
    │   └── stk500-flasher.js      # STK500v1 serial flasher and Intel HEX parser
    └── lib/                       # Browser-side dependencies
```
...
3. Upload an example `.ino` file, such as `Blink.ino`.
4. Enter a title, choose the target board, and click **Compile Sketch & Publish Binary**.
5. Observe the `arduino-cli` stdout/stderr terminal output.
6. Click the **User Catalog** tab to view your newly published binary project.
7. Connect your microcontroller via USB.
8. Click **Connect & Flash**, select your serial COM port, and flash the compiled binary.

---

## 👨‍💻 Developer

Built by **Pratik Kothare** - Full Stack Developer.

If you like this repo, please ⭐ star it on GitHub.
